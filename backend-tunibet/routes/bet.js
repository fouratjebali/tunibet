...
router.get('/last-bets', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT bet_number, amount, created_at, id AS user_id FROM bets WHERE car_id = $1 ORDER BY created_at DESC LIMIT 5;",
      [car_id]
    );
...
router.post('/place-bet', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "INSERT INTO bets (car_id, id, amount) VALUES ($1, $2, $3) RETURNING bet_number, car_id, id AS user_id, amount, created_at",
...
router.post('/accept-bet', async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO accepted_bets (bet_number, id, car_id, amount) VALUES ($1, $2, $3, $4)",
      [bet_number, user_id, car_id, amount]
    );
    
    await pool.query(
      "UPDATE cars SET is_sold = TRUE WHERE car_id = $1",
      [car_id]
    );
